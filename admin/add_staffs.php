...
if(isset($_POST['submit'])){
    ...
    if($error == ""){
        ...
        $email = strilize_input($email);
        // CHECK IF STAFF ALREADY ON DATABASE
        ...
        if(mysqli_num_rows($result_verfy_staff) > 0){
            $error = "Error: [ {$last_name} {$first_name} ] alredy have an account with this email=> ({$email})";
        }else{
            ...
            $sql_input = "INSERT INTO staffs (staff_title, staff_first_name, staff_last_name, staff_mid_name, staff_email, staff_phone_number, staff_course, rank) 
                        VALUES ('$staff_title','$first_name','$last_name','$mid_name','$email','$phone_number','$course','$rank')";
            $result_input = mysqli_query($conn, $sql_input);

            if(!$result_input){
                $error = "Error: {$first_name} with email=> ({$email}) not Added, Please try again!";
            }else{

                $message = "<p>Successful: {$first_name} with email=> ({$email}) Added...</p>";
                $staff_details = $email;
                $staff_title = "";
                $first_name = "";
                $last_name = "";
                ...
                $course = "";
                $message .= "<script>
                            setTimeout(() => window.location = \"staffs.php?message={$staff_details}\", 3000);
                        </script>";
            }
        }
...
<main>
    ...
    <div class="w-full text-center px-6">
        ...
        <div class="p-3 my-3 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-center text-gray-600">Add Staff</h2>
        </div>
        ...
        <div class="p-6 mx-auto my-auto text-left bg-white shadow-lg w-[98%] h-[98%] m-auto">
            <form action="<?php echo $page_name;?>.php?admin" method="post" id="form">
                        ...
                        <div class="mt-4 md:ml-2">
                            <label class="block" for="last_name">Last Name<label>
                                    <input type="text" placeholder="Last Name" id="last_name"
                                        name="last_name"
                                        value="<?php echo (isset($last_name) AND !empty($last_name))? "{$first_name}": "";?>"
                    ...
                    <div class="flex items-center justify-center h-10 intro-y my-6">
                        ...
                        border-[#08a7fd] transition"><i class="fa fa-send m-1 pr-2"></i> Add Staffs
                        </button>
                    </div>
            </form>
